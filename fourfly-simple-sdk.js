...
class FourFlySimpleSDK {
  constructor(config) {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      throw new Error('Configuration Supabase requise: supabaseUrl et supabaseAnonKey');
    }
  ...
  async setUserToken(token) {
    const { data, error } = await this.supabase.auth.setSession({
      access_token: token,
      refresh_token: '' // Optionnel selon votre implémentation
    });
    ...
    return { success: !error, error: error?.message, user: data?.user };
  }
  ...
  async getFlights(options = {}) {
    ...
    if (error) {
      throw new Error(`Erreur lors de la récupération des vols: ${error.message}`);
    }
    ...
    return data.map(flight => ({
      ...
      fuel_used: (flight.fuel_added_before || 0) + (flight.fuel_added_after || 0),
  ...
  async getAircraft() {
    ...
    if (error) {
      throw new Error(`Erreur lors de la récupération des avions: ${error.message}`);
    }
  ...
  async getFlightCarbonData(options = {}) {
    ...
    return flights.map(flight => {
      const co2_kg = this.calculateCO2(flight.fuel_used, flight.emission_factor);
